...
test("Bug issue Filters in woman category not working", async (t) => {
  await t
  .expect(page.home_category_links.nth(0).exists)
  .ok()
  .click(page.home_category_links.nth(0));
  ...
  await t
    .expect(page.tops_checkboxs.visible).ok()
    .click(page.tops_checkboxs)
    .expect(page.tops_checkboxs.checked).ok()

  await t
    .expect(page.loading_gif.visible).ok()
    console.log('Page keeps showing loading gif without returning new results');
});

test("Bug issue Filters in dresses category not working", async (t) => {
  await t
  .expect(page.home_category_links.nth(1).exists)
  .ok()
  .click(page.home_category_links.nth(1));

  await t
    .expect(page.dresses_checkboxs.visible).ok()
    .click(page.dresses_checkboxs)
    .expect(page.dresses_checkboxs.checked).ok()
...
test("Bug issue Filters in t-shirts category not working", async (t) => {
  await t
  .expect(page.home_category_links.nth(2).exists)
  .ok()
  .click(page.home_category_links.nth(2));
  ...
  await t
    .expect(page.tshirt_checkboxs.visible).ok()
    .click(page.tshirt_checkboxs)
    .expect(page.tshirt_checkboxs.checked).ok()
...
test("Bug issue slider filters in t-shirts category not working", async (t) => {
  await t
  .expect(page.home_category_links.nth(2).exists)
  .ok()
  .click(page.home_category_links.nth(2));
  ...
  await t
    .expect(page.price_slider.visible).ok()
    .drag(page.price_slider, 30, 0)
